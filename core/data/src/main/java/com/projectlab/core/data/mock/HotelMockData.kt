...
fun randomHotelRating(stars: Int): HotelRating {
    ...
    return HotelRating(
        overallRating = Random.nextInt(from = 61, until = 99),
        numberOfReviews = Random.nextInt(from = 0, until = 3000),
        stars = stars,
    )
}

fun randomHotelPhotoUrls(): List<String> {
...
fun randomHotelDisplayImageUrl(): String {
    val randomImage = Random.nextInt(from = 0, until = photoUrls.size - 1)
    return photoUrls[randomImage]
}
...
fun randomHotelAmenities(): List<String> {
